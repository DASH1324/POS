...
function EditSpillageModal({ spillage, onClose, onUpdate, loggedByName }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Update spillage record - backend handles inventory adjustment in background
      const response = await fetch(
      ...
      if (response.ok) {
        ...
        console.log("Spillage updated successfully. Inventory adjustment processing in background.");
